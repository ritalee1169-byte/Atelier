(function(global, factory) {
	...
	***************************************************************************** */
	...
	function useMachine(stateMachine, options) {
		const persistedStateRef = (0, react.useRef)();
		const [service, queue] = useConstant(() => {
			const eventQueue = [];
			const svc = v(s(stateMachine.config, options ? options : stateMachine._options));
		...
		useIsomorphicLayoutEffect(() => {
			if (options) service._machine._options = options;
		});
		const getSnapshot = (0, react.useCallback)(() => getServiceState(service), [service]);
		const storeSnapshot = (0, react.useSyncExternalStore)((0, react.useCallback)((handleStoreChange) => {
...
`;
	//#endregion
	//#region src/storefront-api-constants.ts
	...
	*/
	...
	var ShopifyContext = (0, react.createContext)(defaultShopifyContext);
	/**
	...
	*/
	function ShopifyProvider({ children, ...shopifyConfig }) {
		...
		const finalConfig = (0, react.useMemo)(() => {
	...
	*/
	function useShop() {
		const shopContext = (0, react.useContext)(ShopifyContext);
		if (!shopContext) throw new Error(`'useShop()' must be a descendent of <ShopifyProvider/>`);
		return shopContext;
	}
	//#endregion
	//#region src/cart-constants.ts
	...
	*/
	...
	function useCartFetch() {
		const { storefrontId, getPublicTokenHeaders, getStorefrontApiUrl, sameDomainForStorefrontApi } = useShop();
		return (0, react.useCallback)(({ query, variables }) => {
			const headers = getPublicTokenHeaders({ contentType: "json" });
			if (storefrontId) headers[SHOPIFY_STOREFRONT_ID_HEADER] = storefrontId;
	...
	*/
	function useCartActions({ numCartLines, cartFragment, countryCode = "US", languageCode = "EN" }) {
		const fetchCart = useCartFetch();
		const cartFetch = (0, react.useCallback)((cartId) => {
			return fetchCart({
		...
		const cartCreate = (0, react.useCallback)((cart) => {
			return fetchCart({
		...
		const cartLineAdd = (0, react.useCallback)((cartId, lines) => {
			return fetchCart({
		...
		const cartLineUpdate = (0, react.useCallback)((cartId, lines) => {
			return fetchCart({
		...
		const cartLineRemove = (0, react.useCallback)((cartId, lines) => {
			return fetchCart({
		...
		const noteUpdate = (0, react.useCallback)((cartId, note) => {
			return fetchCart({
		...
		const buyerIdentityUpdate = (0, react.useCallback)((cartId, buyerIdentity) => {
			return fetchCart({
		...
		const cartAttributesUpdate = (0, react.useCallback)((cartId, attributes) => {
			return fetchCart({
		...
		const discountCodesUpdate = (0, react.useCallback)((cartId, discountCodes) => {
			return fetchCart({
	...
	function invokeCart(action, options) {
		return {
			entry: [
				...
				i({ lastValidCart: (context) => context?.cart }),
				"onCartActionEntry",
	...
	function useCartAPIStateMachine({ numCartLines, onCartActionEntry, onCartActionOptimisticUI, onCartActionComplete, data: cart, cartFragment, countryCode, languageCode }) {
		...
		const [state, send, service] = useMachine((0, react.useMemo)(() => createCartMachine(cart), [cart]), { actions: {
			...
			...onCartActionOptimisticUI && { onCartActionOptimisticUI: i((context, event) => {
	...
	*/
	...
	var ProductOptionsContext = (0, react.createContext)(null);
	/**
	...
	*/
	function ProductProvider({ children, data: product, initialVariantId: explicitVariantId }) {
		const variants = (0, react.useMemo)(() => flattenConnection(product.variants ?? {}), [product.variants]);
		...
		const options = (0, react.useMemo)(() => getOptions(variants), [variants]);
		/**
		* Track the selectedVariant within the provider.
		*/
		const [selectedVariant, setSelectedVariant] = (0, react.useState)(() => getVariantBasedOnIdProp(explicitVariantId, variants));
		/**
		* Track the selectedOptions within the provider. If a `initialVariantId`
		* is passed, use that to select initial options.
		*/
		const [selectedOptions, setSelectedOptions] = (0, react.useState)(() => getSelectedOptions(selectedVariant));
		/**
		...
		*/
		const setSelectedOption = (0, react.useCallback)((name, value) => {
		...
		const isOptionInStock = (0, react.useCallback)((option, value) => {
			return getSelectedVariant(variants, {
				...selectedOptions,
				[option]: value
			})?.availableForSale ?? true;
		}, [selectedOptions, variants]);
		const sellingPlanGroups = (0, react.useMemo)(() => flattenConnection(product.sellingPlanGroups ?? {}).map((sellingPlanGroup) => ({
			...sellingPlanGroup,
		...
		*/
		const [selectedSellingPlan, setSelectedSellingPlan] = (0, react.useState)(void 0);
		const selectedSellingPlanAllocation = (0, react.useMemo)(() => {
	...
	*/
	function useProduct() {
		const context = (0, react.useContext)(ProductOptionsContext);
	...
	function BaseButton(props) {
		...
		const handleOnClick = (0, react.useCallback)((event) => {
			if (onClick) {
				...
				if (typeof clickShouldContinue === "boolean" && clickShouldContinue === false || event?.defaultPrevented) return;
			}
	...
	//#endregion
	//#region src/AddToCartButton.tsx
	/**
	...
	*/
	function AddToCartButton(props) {
		const [addingItem, setAddingItem] = (0, react.useState)(false);
		const { variantId: explicitVariantId, quantity = 1, attributes, sellingPlanId, onClick, children, accessibleAddingToCartLabel, parent, ...passthroughProps } = props;
		...
		(0, react.useEffect)(() => {
			if (addingItem && status === "idle") setAddingItem(false);
		}, [status, addingItem]);
		const handleAddItem = (0, react.useCallback)(() => {
			setAddingItem(true);
			linesAdd([{
				quantity,
		...
		return /* @__PURE__ */ react.createElement(react.Fragment, null, /* @__PURE__ */ react.createElement(BaseButton, {
			...passthroughProps,
			disabled,
			onClick,
			defaultOnClick: handleAddItem
		}, children), accessibleAddingToCartLabel ? /* @__PURE__ */ react.createElement("p", {
			style: {
				position: "absolute",
				width: "1px",
				height: "1px",
	...
	*/
	...
	//#endregion
	//#region src/analytics-schema-custom-storefront-customer-tracking.ts
	...
	* @publicDocs
	*/
	var ExternalVideo = (0, react.forwardRef)((props, ref) => {
...
`;
	...
	*/
	var Image = react.forwardRef(({ alt, aspectRatio, crop = "center", data, decoding = "async", height = "auto", loader = shopifyLoader, loading = "lazy", sizes, src, srcSetOptions = {
		...
		if (fixedWidth) return /* @__PURE__ */ react.createElement(FixedWidthImage, {
			aspectRatio,
			crop,
			decoding,
			height,
		...
		else return /* @__PURE__ */ react.createElement(FluidImage, {
			aspectRatio,
			crop,
			decoding,
			imageWidths,
	...
	* @publicDocs
	*/
	var Video = (0, react.forwardRef)((props, ref) => {
	...
	*/
	function ModelViewer(props) {
		...
		return /* @__PURE__ */ React.createElement("model-viewer", {
			ref: callbackRef,
			...passthroughProps,
			class: className,
	...
	*/
	function MediaFile({ data, mediaOptions, ...passthroughProps }) {
		switch (data.__typename) {
			...
			case "Video": return /* @__PURE__ */ React.createElement(Video, {
				...passthroughProps,
				...mediaOptions?.video,
				data
			});
			case "ExternalVideo": return /* @__PURE__ */ React.createElement(ExternalVideo, {
				...passthroughProps,
				...mediaOptions?.externalVideo,
				data
			});
			case "Model3d": return /* @__PURE__ */ React.createElement(ModelViewer, {
				...passthroughProps,
	...
	*/
	function ProductPrice(props) {
		...
		if (measurement) return /* @__PURE__ */ React.createElement(Money, {
			...passthroughProps,
			data: price,
			measurement
		});
	...
	function Text({ node }) {
		if (node.bold && node.italic) return /* @__PURE__ */ React.createElement("em", null, /* @__PURE__ */ React.createElement("strong", null, node.value));
	...
	function List({ node }) {
		...
		return /* @__PURE__ */ React.createElement(List, null, node.children);
	}
	...
	function RichText({ as, data, plain, components, ...passthroughProps }) {
		try {
			const Wrapper = as ?? "div";
			const parsedData = (0, react.useMemo)(() => JSON.parse(data), [data]);
	...
	function serializeRichTextASTNode(components = {}, node, index = 0) {
		...
		switch (node.type) {
			case "root": return (0, react.createElement)(Component, {
				key: index,
				node: {
					type: "root",
					children
			...
			case "heading": return (0, react.createElement)(Component, {
				key: index,
				node: {
					type: "heading",
					level: node.level,
			...
			case "paragraph": return (0, react.createElement)(Component, {
				key: index,
				node: {
					type: "paragraph",
					children
			...
			case "text": {
				const elements = (node.value ?? "").split("\n").flatMap((value, subindex) => {
					...
					const textElement = (0, react.createElement)(Component, {
						key,
						node: {
							type: "text",
							italic: node.italic,
					...
					return subindex === 0 ? textElement : [(0, react.createElement)("br", { key: `${key}-br` }), textElement];
				});
				return elements.length > 1 ? (0, react.createElement)(react.Fragment, { key: index }, elements) : elements[0];
			}
			case "link": return (0, react.createElement)(Component, {
				key: index,
				node: {
					type: "link",
					url: node.url,
			...
			case "list": return (0, react.createElement)(Component, {
				key: index,
				node: {
					type: "list",
			...
			case "list-item": return (0, react.createElement)(Component, {
				key: index,
				node: {
					type: "list-item",
					children
	...
	*/
	function ShopPayButton({ channel, variantIds, className, variantIdsAndQuantities, width, storeDomain: _storeDomain }) {
		...
		const style = width ? { "--shop-pay-button-width": width } : void 0;
		return /* @__PURE__ */ React.createElement("div", {
			className,
			style
		}, shopPayLoadedStatus === "done" && /* @__PURE__ */ React.createElement("shop-pay-button", {
	...
	//#endregion
	//#region src/useSelectedOptionInUrlParam.tsx
	/** @publicDocs */
